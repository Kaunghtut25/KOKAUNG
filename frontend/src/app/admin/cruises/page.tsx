...
export default function AdminCruisesPage() {
  ...
  const [editingCruise, setEditingCruise] = useState<cruise>(emptyCruise);
  ...
  const fetchCruises = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/admin/cruises`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
        const data = await res.json();
        setCruises(Array.isArray(data) ? data : data.cruises || []);
      }
    } catch (err) {
  ...
  const openEditModal = (cruise: cruise) => {
    setEditingCruise({ ...cruise });
    const imgs = parseImageList(cruise.images);
    setImageList(imgs);
  ...
  const handleFieldChange = (field: keyof cruise, value: string | number) => {
    setEditingCruise((prev) => ({ ...prev, [field]: value }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = isNew
        ? `${API_BASE}/admin/cruises`
        : `${API_BASE}/admin/cruises/${editingCruise.id}`;
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify(editingCruise),
      });
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await fetch(`${API_BASE}/admin/cruises/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input
            type="text"
            value={editingCruise.destination}
            onChange={(e) =>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input
            type="number"
            value={editingCruise.maxGroupSize}
            onChange={(e) =>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="flex items-end pb-0.5">
          <label className="flex items-center gap-3 cursor-pointer">
            <input
              type="checkbox"
              checked={editingCruise.featured}
              onChange={(e) =>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {cruises.length === 0 ? (
                ...
                cruises.map((cruise) => {
                  const thumb = getFirstImage(cruise.images);
                  return (
                    <tr
                      key={cruise.id}
                    ...
                    >
                      ...
                      <td className="p-4 text-white">
                        {formatNumber(cruise.priceMMK)} Ks
                      </td>
                      <td className="p-4 text-white">
                        ${formatNumber(cruise.priceUSD)}
                      </td>
                      <td className="p-4 text-white/70">{cruise.duration}</td>
                      <td className="p-4">
                        <div className="flex items-center gap-1.5">
                          <span style={getStatusBadge(cruise.status)} className="px-2 py-0.5 rounded-full text-xs font-medium border">
