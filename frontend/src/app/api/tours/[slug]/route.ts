...
function transformTour(t: Record<string, unknown>) {
  ...
  return {
    ...
    priceMMK: Number(t.priceMMK) > 0 ? Number(t.priceMMK) : 0,
    priceUSD: Number(t.priceUSD) > 0 ? Number(t.priceUSD) : 0,
    quoteRequired: Boolean(t.quote_required || t.quoteRequired) || (!Number(t.priceMMK) && !Number(t.priceUSD)),
    duration: (t.duration as string) || 'N/A',
    durationUnit: 'days',
    groupSize: Number(t.maxGroupSize) || Number(t.capacity) || 10,
    rating: Number(t.rating) > 0 ? Number(t.rating) : 0,
    reviewCount: Number(t.reviewCount) > 0 ? Number(t.reviewCount) : 0,
