  ...
  return (
    <div style={{ background: 'linear-gradient(135deg,#0A1628,#0F2035)', borderBottom: '3px solid #D4AF37', padding: '24px 20px', textAlign: 'center' }}>
      <div style={{ maxWidth: 800, margin: '0 auto' }}>
        <span style={{ color: '#D4AF37', fontSize: 12, fontWeight: 700, letterSpacing: 1 }}>{deals.badge}</span>
        <h2 style={{ color: 'white', fontSize: 24, fontWeight: 700, margin: '8px 0' }}>{deals.title}</h2>
        ...
        <a href={deals.buttonHref} style={{ display: 'inline-block', background: '#D4AF37', color: '#0A1628', padding: '10px 28px', borderRadius: 24, textDecoration: 'none', fontWeight: 700, fontSize: 14 }}>{deals.buttonLabel}</a>
      </div>
    </div>
  );
}
