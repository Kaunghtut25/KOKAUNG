    ...
    <div className="bg-white rounded-xl p-6 text-center shadow-md hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
      {imgSrc ? (
        <div className="w-full h-32 mx-auto mb-4 relative rounded-lg overflow-hidden bg-gray-50">
          <Image src={imgSrc} alt={title} fill sizes="(max-width: 768px) 50vw, 25vw" className="object-contain" />
        </div>
      ) : (
      ...
      {showCalendar && (
        <div className="absolute top-full left-0 mt-1 z-50 bg-white border border-gray-200 rounded-xl shadow-2xl p-4 w-72" style={{minWidth:"280px"}}>
          ...
          <div className="grid grid-cols-7 mb-1">
            {["Su","Mo","Tu","We","Th","Fr","Sa"].map(d => <div key={d} className="text-center text-[10px] font-medium text-gray-500 py-1">{d}</div>)}
          </div>
          {/* Day grid */}
...
function CityDropdown({ cities, value, onChange, placeholder }: { cities: Array<{ city: string; cityMY: string; region: string; popular: boolean }>; value: string; onChange: (v: string) => void; placeholder: string }) {
  ...
  const filtered = query ? cities.filter(c => c.city.toLowerCase().includes(query.toLowerCase()) || c.cityMY.includes(query)) : cities;
          ...
          <div className="max-h-36 overflow-y-auto">
            {filtered.map(c => (
              <div key={c.city} onClick={() => { onChange(c.city); setOpen(false); setQuery(""); }}
                className={"px-4 py-2 text-sm cursor-pointer hover:bg-[#D4AF37]/10 border-b border-gray-50 last:border-0 " + (c.city === value ? "bg-[#D4AF37]/20 font-semibold" : "text-gray-700")}>
                {c.popular ? "★ " : ""}{c.city} ({c.cityMY}) — {c.region}
              </div>
            ))}
            {filtered.length === 0 && <div className="px-4 py-2 text-sm text-gray-500">{t("home.noCities")}</div>}
          </div>
        </div>
      ) : (
        <button type="button" onClick={() => setOpen(true)}
          className="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 text-sm text-left outline-none focus:border-[#D4AF37] transition-all cursor-pointer">
          {selected ? <>{selected.popular ? "★ " : ""}{selected.city} ({selected.cityMY}) — {selected.region}</> : <span className="text-gray-500">{placeholder}</span>}
        </button>
      )}
    </div>
  );
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <section className="relative w-full overflow-hidden" style={{ height: `${siteConfig?.heroHeightDesktop || 460}px`, maxHeight: "100vh" }}>
        ...
        <button onClick={prevSlideHandler} aria-label="Previous slide" className="absolute left-4 top-1/2 -translate-y-1/2 z-20 w-12 h-12 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 flex items-center justify-center text-white hover:bg-white/20 transition-all cursor-pointer">
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
        </button>
        <button onClick={nextSlide} aria-label="Next slide" className="absolute right-4 top-1/2 -translate-y-1/2 z-20 w-12 h-12 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 flex items-center justify-center text-white hover:bg-white/20 transition-all cursor-pointer">
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
        </button>
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-30 flex gap-3">
          {slides.map((_:any, index:number) => (
            <button key={index} onClick={() => goToSlide(index)}
              className="min-w-[48px] min-h-[48px] flex items-center justify-center rounded-full transition-all duration-300 cursor-pointer group"
              ...
              <form onSubmit={handleSearch} className="space-y-3">
                {activeTab==="multicity" ? (<>
                  ...
                  <div className="flex flex-col md:flex-row gap-3 items-end flex-wrap">
                    ...
                    <div className="w-full md:w-[180px]"><label className="block text-gray-500 text-xs uppercase tracking-wider mb-1"><span className="text-[#D4AF37]">💺</span> {t("home.class")}</label><select value={travelClass} onChange={e=>setTravelClass(e.target.value)} aria-label={t("home.class")} className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-3 text-gray-900 outline-none focus:border-[#D4AF37] transition-all cursor-pointer"><option value="Economy">Economy</option><option value="Premium Economy">Premium Economy</option><option value="Business">Business</option><option value="First">First Class</option></select></div>
                    <div className="w-full md:w-[180px]"><label className="block text-gray-500 text-xs uppercase tracking-wider mb-1"><span className="text-[#D4AF37]">🛂</span> {t("home.client")}</label><div className="flex bg-gray-50 border border-gray-200 rounded-lg p-1"><button type="button" onClick={()=>setClientType('local')} className={`flex-1 py-1.5 text-xs font-medium min-h-[48px] flex items-center justify-center rounded-md transition-all ${clientType==='local'?'bg-[#D4AF37] text-[#0A1628] shadow-sm':'text-gray-500 hover:text-gray-900'}`}><FlagIcon code="mm" width={16} className="inline-block mr-1" /> {t("home.local")}</button><button type="button" onClick={()=>setClientType('foreigner')} className={`flex-1 py-1.5 text-xs font-medium min-h-[48px] flex items-center justify-center rounded-md transition-all ${clientType==='foreigner'?'bg-[#D4AF37] text-[#0A1628] shadow-sm':'text-gray-500 hover:text-gray-900'}`}>🌏 {t("home.foreigner")}</button></div></div>
                    <button type="submit" className="w-full md:w-auto bg-gradient-to-r from-[#D4AF37] to-[#F5A623] text-[#0A1628] font-bold px-8 py-3 rounded-lg hover:shadow-xl hover:shadow-[#D4AF37]/40 hover:scale-105 active:scale-95 transition-all duration-300 cursor-pointer">🔍 {t("home.searchFlights")}</button>
                  </div>
                </>) : (<>
                  <div className="grid grid-cols-1 md:flex md:flex-row gap-3">
                    ...
                    <button type="button" onClick={swapAirports} className="hidden md:flex items-center justify-center w-12 h-12 mt-6 rounded-full bg-gray-100 border border-gray-200 text-gray-600 hover:text-gray-600 hover:border-[#D4AF37]/50 transition-all cursor-pointer flex-shrink-0" title={t("home.swapAirports")}>⇄</button>
                ...
                <div className="flex flex-col md:flex-row gap-3 items-end flex-wrap">
                  ...
                  <div className="w-full md:w-[180px]">
                    ...
                    <div className="flex bg-gray-50 border border-gray-200 rounded-lg p-1">
                      <button type="button" onClick={()=>setBusClientType('local')} className={busClientType==='local'?'flex-1 py-1.5 text-xs font-medium min-h-[48px] flex items-center justify-center rounded-md bg-[#D4AF37] text-white shadow-sm transition-all':'flex-1 py-1.5 text-xs font-medium min-h-[48px] flex items-center justify-center rounded-md text-gray-500 hover:text-gray-900 transition-all'}><FlagIcon code="mm" width={16} className="inline-block mr-1" /> {t("home.local")}</button>
                      <button type="button" onClick={()=>setBusClientType('foreigner')} className={busClientType==='foreigner'?'flex-1 py-1.5 text-xs font-medium min-h-[48px] flex items-center justify-center rounded-md bg-[#D4AF37] text-white shadow-sm transition-all':'flex-1 py-1.5 text-xs font-medium min-h-[48px] flex items-center justify-center rounded-md text-gray-500 hover:text-gray-900 transition-all'}>🌏 {t("home.foreigner")}</button>
                    </div>
                  </div>
      ...
      <section className="py-24 bg-[#0A1628] relative overflow-hidden">
        {ctaBgImage && <Image src={ctaBgImage} alt="A9 Global Travels showcase background" fill sizes="100vw" className="object-cover opacity-10" aria-hidden />}
