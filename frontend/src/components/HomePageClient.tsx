...
function BusDropdown({ cities, value, onChange, placeholder }: { cities: Array<{ city: string; cityMY: string; region: string; popular: boolean }>; value: string; onChange: (v: string) => void; placeholder: string }) {
  ...
  return (
    <div ref={ref} className="relative">
      {open ? (
        <>
          <input autoFocus value={query} onChange={e => setQuery(e.target.value)} placeholder={"Search " + placeholder.toLowerCase() + "..."} className="w-full bg-gray-50 border border-[#D4AF37] rounded-lg px-4 py-3 text-sm text-gray-900 outline-none" />
          <div className="absolute top-full left-0 right-0 z-50 mt-1 bg-white border border-gray-200 rounded-lg shadow-xl max-h-36 overflow-y-auto">
            {filtered.map(c => (
              <div key={c.city} onClick={() => { onChange(c.city); setOpen(false); setQuery(""); }} className={"px-4 py-2 text-sm cursor-pointer hover:bg-[#D4AF37]/10 border-b border-gray-100 last:border-0 " + (c.city === value ? "bg-[#D4AF37]/20 font-semibold" : "text-gray-700")}>
        ...
        <button type="button" onClick={() => setOpen(true)} className="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 text-sm text-gray-900 outline-none focus:border-[#D4AF37] transition-all cursor-pointer text-left">
          {selected ? <>{selected.popular ? "★ " : ""}{selected.city} ({selected.cityMY}) — {selected.region}</> : <span className="text-gray-400">{placeholder}</span>}
        </button>
      )}
    </div>
  );
                ...
                {activeTab==="multicity" ? (<>
                  <div className="space-y-4">
                    {multiCityLegs.map((leg,index)=><div key={index} className="flex flex-col sm:flex-row items-start gap-2 sm:gap-3">
                      ...
                      <div className="flex-1 min-w-[140px]"><label className="block text-gray-500 text-xs uppercase tracking-wider mb-1 flex items-center gap-1"><span className="text-[#D4AF37]">📅</span>Date</label><input type="date" value={leg.date} onChange={e=>updateMultiCityLeg(index,"date",e.target.value)} min={new Date().toISOString().split("T")[0]} className="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 text-gray-900 outline-none focus:border-[#D4AF37] transition-all" /></div>
            ...
            {searchMode==='buses' && (()=>{const bp=busPassengers;const totalPax=bp.adults+bp.children;return (
              <form onSubmit={(e)=>{e.preventDefault();try{const events=JSON.parse(localStorage.getItem('a9_search_submit_events')||'[]');events.push({type:'bus',from:busFrom,to:busTo,date:busDate,adults:bp.adults,children:bp.children,client:busClientType,timestamp:new Date().toISOString()});if(events.length>100)events.splice(0,events.length-100);localStorage.setItem('a9_search_submit_events',JSON.stringify(events))}catch(e){};router.push('/book-now?type=bus&from='+encodeURIComponent(busFrom)+'&to='+encodeURIComponent(busTo)+'&date='+encodeURIComponent(busDate)+'&adults='+bp.adults+'&children='+bp.children+'&client='+encodeURIComponent(busClientType))}} className="space-y-3">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div><label className="block text-gray-500 text-xs uppercase tracking-wider mb-1">🚍 From City</label><BusDropdown cities={busCities} value={busFrom} onChange={setBusFrom} placeholder="Select city" /></div>
                  <div><label className="block text-gray-500 text-xs uppercase tracking-wider mb-1">🚍 To City</label><BusDropdown cities={busCities.filter(c => c.city !== busFrom)} value={busTo} onChange={setBusTo} placeholder="Select city" /></div>
