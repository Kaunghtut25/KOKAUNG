            ...
            {searchMode==='buses' && (()=>{const bp=busPassengers;const totalPax=bp.adults+bp.children;return (
              <form onSubmit={(e)=>{e.preventDefault();try{const events=JSON.parse(localStorage.getItem('a9_search_submit_events')||'[]');events.push({type:'bus',from:busFrom,to:busTo,date:busDate,adults:bp.adults,children:bp.children,client:busClientType,timestamp:new Date().toISOString()});if(events.length>100)events.splice(0,events.length-100);localStorage.setItem('a9_search_submit_events',JSON.stringify(events))}catch(e){};router.push('/book-now?type=bus&from='+encodeURIComponent(busFrom)+'&to='+encodeURIComponent(busTo)+'&date='+encodeURIComponent(busDate)+'&adults='+bp.adults+'&children='+bp.children+'&client='+encodeURIComponent(busClientType))}} className="space-y-3">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  ...
                  <div>
  <label className="block text-gray-500 text-xs uppercase tracking-wider mb-1">📅 Travel Date</label>
  {(() => {
    const today = new Date();
    const [yy, setYY] = useState(today.getFullYear());
    const [mm, setMM] = useState(today.getMonth());
    ...
    const selectedDate = dd ? new Date(yy, mm, dd).toISOString().split("T")[0] : "";
    ...
    // sync busDate
    useEffect(() => { if (selectedDate) setBusDate(selectedDate); }, [selectedDate]);
    useEffect(() => {
      const h = (e: MouseEvent) => {
        const t = e.target as HTMLElement;
        if (!t.closest(".cal-popup")) setShow(false);
      };
    ...
    return (
      <div className="relative cal-popup">
        <button type="button" onClick={() => setShow(!show)}
          className="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 text-sm text-left outline-none focus:border-[#D4AF37] transition-all cursor-pointer flex items-center justify-between group hover:border-gray-300">
          <span className={dd ? "text-gray-900 font-medium" : "text-gray-400"}>
            {dd ? formatDate(dd) : "Select travel date"}
          </span>
          <svg className="w-4 h-4 text-[#D4AF37] group-hover:scale-110 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {show && (
          <div className="absolute top-full left-0 mt-1 z-50 bg-white border border-gray-200 rounded-xl shadow-2xl p-3 w-72">
            ...
            <div className="flex items-center justify-between mb-3">
              <button onClick={() => { if (mm === 0) { setMM(11); setYY(yy - 1); } else setMM(mm - 1); }}
              ...
              <span className="text-sm font-semibold text-gray-800">{months[mm]} {yy}</span>
              <button onClick={() => { if (mm === 11) { setMM(0); setYY(yy + 1); } else setMM(mm + 1); }}
                className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 text-gray-600 transition-colors">
