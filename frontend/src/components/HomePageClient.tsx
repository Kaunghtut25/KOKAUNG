            ...
            {searchMode==='buses' && (()=>{const bp=busPassengers;const totalPax=bp.adults+bp.children;return (
              <form onSubmit={(e)=>{e.preventDefault();try{const events=JSON.parse(localStorage.getItem('a9_search_submit_events')||'[]');events.push({type:'bus',from:busFrom,to:busTo,date:busDate,adults:bp.adults,children:bp.children,client:busClientType,timestamp:new Date().toISOString()});if(events.length>100)events.splice(0,events.length-100);localStorage.setItem('a9_search_submit_events',JSON.stringify(events))}catch(e){};router.push('/book-now?type=bus&from='+encodeURIComponent(busFrom)+'&to='+encodeURIComponent(busTo)+'&date='+encodeURIComponent(busDate)+'&adults='+bp.adults+'&children='+bp.children+'&client='+encodeURIComponent(busClientType))}} className="space-y-3">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div><label className="block text-gray-500 text-xs uppercase tracking-wider mb-1">🚍 From City</label><select value={busFrom} onChange={e=>setBusFrom(e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-900 outline-none focus:border-[#D4AF37] transition-all cursor-pointer" required><option value="">Select city</option>{busCities.map(c=><option key={"from-"+c.city} value={c.city}>{c.popular?'★ ':''}{c.city} ({c.region})</option>)}</select></div>
                  <div><label className="block text-gray-500 text-xs uppercase tracking-wider mb-1">🚍 To City</label><select value={busTo} onChange={e=>setBusTo(e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-900 outline-none focus:border-[#D4AF37] transition-all cursor-pointer" required><option value="">Select city</option>{busCities.map(c=><option key={"to-"+c.city} value={c.city}>{c.popular?'★ ':''}{c.city} ({c.region})</option>)}</select></div>
